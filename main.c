...
void introsort(Item *arr, int n);

// Funções auxiliares
void merge(int *v, int l, int m, int r);
void fixDown(int k, int N);
int partition(int *v, int l, int r);
void introsort_util(Item *arr, int l, int r, int depth_limit);
...
int main(int argc, char *argv[]) {
    ...
    if (strcmp(argv[1], "heapsort") == 0) {
        ...
        insertion_sort(v, n);
    } else if (strcmp(argv[1], "introsort") == 0) {
        introsort(v, n);
    } else {
...
// Função de ordenação por introsort
void introsort(Item *arr, int n) {
...
void introsort_util(Item *arr, int l, int r, int depth_limit) {
    ...
    introsort_util(arr, l, p - 1, depth_limit - 1);
    introsort_util(arr, p + 1, r, depth_limit - 1);
}